          ...
          {clientsWithProgress.length > 0 ? (
            clientsWithProgress.map((client) => {
              ...
              const calorieProgress =
                client.kcal_goal && log
                  ? Math.min(
                      (log.total_kcal / client.kcal_goal) * 100,
                      100
                    )
                  : 0;
              const proteinProgress =
                client.protein_units && log
                  ? Math.min(
                      (log.total_protein_units / client.protein_units) * 100,
                      100
                    )
                  : 0;
              const carbProgress =
                client.carb_units && log
                  ? Math.min(
                      (log.total_carb_units / client.carb_units) * 100,
                      100
                    )
                  : 0;
              const fatProgress =
                client.fat_units && log
                  ? Math.min(
                      (log.total_fat_units / client.fat_units) * 100,
                      100
                    )
                  : 0;
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    logger.info(
                      "[AdminClients] Navigating to user dashboard:",
                      client.user_id,
                      client.name
                    );
                    router.push({
                ...
                >
                  <View style={styles.clientHeader}>
                    {client.whatsapp_link && (
                      ...
                      >
                        <Image
                          source={{
                            uri: "https://res.cloudinary.com/dtffqhujt/image/upload/v1760615137/whatsapp_2_qkcamm.png",
                          }}
                  ...
                  <View style={styles.progressBars}>
                    <View style={styles.progressItem}>
                      <View style={styles.progressLabelRow}>
                        <Text style={styles.progressValue}>
                          {log ? Math.round(log.total_kcal) : 0}/
                          {client.kcal_goal || 0}
                        </Text>
                    ...
                    <View style={styles.progressItem}>
                      <View style={styles.progressLabelRow}>
                        <Text style={styles.progressValue}>
                          {log ? log.total_protein_units : 0}/
                          {client.protein_units || 0}
                        </Text>
